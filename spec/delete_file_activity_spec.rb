...
RSpec.describe DeleteFileActivity do
  let(:subject) { DeleteFileActivity.new(attributes, logger) }
  ...
  describe "#run" do
    it 'spawns a process with the given arguments' do
        expect(Process).to receive(:spawn).
          with("rm #{path}", out: '/dev/null', err: '/dev/null').and_return(pid)
